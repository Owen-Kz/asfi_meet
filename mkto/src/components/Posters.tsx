...
import {toolbarItemPostersText, toolbarItemSettingText} from '../language/default-labels/videoCallScreenLabels';
...
const Posters = (props: PostersIconButtonProps) => {
  const {sidePanel, setSidePanel} = useSidePanel();
  const {isToolbarMenuItem} = useToolbarMenu();
  //commented for v1 release
  const PostersLLabel = useString(toolbarItemPostersText)();

  const isPanelActive = sidePanel === SidePanelType.Posters;
  const onPress = () => {
    isPanelActive
