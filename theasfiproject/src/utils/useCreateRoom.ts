...
export default function useCreateRoom(): createRoomFun {
  ...
  return async (
    ...
    console.log('LOG CHANNEL RES', res);
    if (res && res?.data && res?.data?.createChannel) {
      ...
      if (res?.data?.createChannel?.passphrase?.channel) {
        roomInfo.channel = res.data.createChannel.channel;
      }
      ...
      fetch('https://asfischolar.net/createMeeting', {
        method: 'POST',
        ...
        .then(data => {
          window.location.href = `https://asfischolar.net/v9/m/${data.channelSecret}`;
