...
export const GetMeetingInviteURL = (
  ...
  if (isHost) {
    if (isSeparateHostLink) {
      return {
        host: `https://asfischolar.net/v3/${roomId.host}`,
        attendee: `https://asfischolar.net/v3/${roomId.attendee}`,
      };
    } else {
      return {
        host: `https://asfischolar.net/v3/${roomId.host}`,
...
const ShareLinkProvider = (props: ShareLinkProvideProps) => {
  ...
  const getAttendeeURLOrId = () => {
    ...
    if (roomId?.attendee) {
      if (baseURL) {
        stringToCopy += `https://asfischolar.net/v3/${roomId.attendee}`;
      } else {
  ...
  const getHostUrlOrId = () => {
    ...
    if (roomId?.host) {
      let baseURL = getBaseURL();
      if (baseURL) {
        stringToCopy += `https://asfischolar.net/v3/${roomId.host}`;
      } else {
